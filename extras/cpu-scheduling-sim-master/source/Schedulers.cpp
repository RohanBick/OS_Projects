...
void Scheduler::pp() {
    ...
    if (timer <= 0 || cpu->isidle()) {
        for (int index = 0; index < ready_queue->size(); ++index) {
            PCB* pcb = ready_queue->getindex(index);
            int temp_prio = pcb->priority;
    ...
    if (low_index >= 0) {
        ...
        dispatcher->interrupt();
        timer = 2 * timeq; // Reset the time quantum (twice the time quantum for CPU burst)
    }
    else {
        timer -= 0.5; // Decrement the time quantum
    }
}
...
void Scheduler::pr() {
    if (cpu->isidle() || timer <= 0) {
        if (ready_queue->size() > 0) {
            // Generate a random index within the range of the ready queue
